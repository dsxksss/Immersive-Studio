...
(() => {
    ...
    let lang = params.get("lang") === "en" ? "en" : ((localStorage.getItem("spatial:lang") || "zh") === "en" ? "en" : "zh");
    ...
    const artboardThumbCache = new Map();
    const artboardThumbTimers = new Map();
    let selected = null;
    ...
    function setGridSnapEnabled(next) {
        gridSnapEnabled = next;
        try {
            localStorage.setItem(GRID_SNAP_KEY, next ? "1" : "0");
        }
        catch { }
    }
    function snapCoord(n) { return Math.round(n / GRID) * GRID; }
    function snapSize(n, min) { return Math.max(min, snapCoord(n)); }
    ...
    function setMouseMode(next) {
        ...
        try {
            localStorage.setItem(MOUSE_MODE_KEY, mouseMode);
        }
        catch { }
        syncMouseModeUi();
    }
    function syncMouseModeUi() {
        ...
        root.querySelectorAll("[data-mouse-mode]").forEach((btn) => {
            const mode = btn.dataset.mouseMode;
            btn.classList.toggle("is-active", mode === mouseMode);
            btn.setAttribute("aria-pressed", mode === mouseMode ? "true" : "false");
        });
    ...
    const richPending = new Map();
    const richTitleDrafts = new Map();
    const richTitlePending = new Map();
    ...
    function persistSavedRoom(entry) {
    ...
    function scheduleTextUpdate(object) {
        if (object.kind === "richText")
            richDrafts.set(object.id, object.content ?? "");
        const prev = textUpdateTimers.get(object.id);
        if (prev)
            clearTimeout(prev);
        textUpdateTimers.set(object.id, setTimeout(() => {
            textUpdateTimers.delete(object.id);
            if (objectById(object.id) !== object)
                return;
            saveObjects();
            if (object.kind === "richText")
                flushRichDraft(object);
            else
                sendUpdate([object]);
        }, TEXT_UPDATE_MS));
    }
    function flushTextUpdate(object) {
        if (objectById(object.id) !== object)
            return;
        const prev = textUpdateTimers.get(object.id);
        if (prev) {
    ...
    function sendRichOperation(object, action, text) {
        if (!collaborationReady || socket?.readyState !== WebSocket.OPEN)
            return false;
        send({ type: "rich-text-operation", objectId: object.id, operation: { action, text } });
        return true;
    }
    function flushRichDraft(object) {
        const draft = richDrafts.get(object.id);
        if (draft !== undefined && draft !== richPending.get(object.id) && sendRichOperation(object, "content", draft))
            richPending.set(object.id, draft);
        const title = richTitleDrafts.get(object.id);
        if (title !== undefined && title !== richTitlePending.get(object.id) && sendRichOperation(object, "title", title))
            richTitlePending.set(object.id, title);
    }
    function setEditingObject(id) { editingObjectId = id; }
    function getActiveEditingId() {
        if (editingObjectId)
    ...
    function rememberRoomSession(session) {
        if (!session.token)
            return;
        const saved = { token: session.token, nickname: session.nickname || me.name, color: session.color || me.color, roomName, expiresAt: Date.now() + 7 * 24 * 3600 * 1000 };
        try {
            localStorage.setItem("spatial:session:" + roomId, JSON.stringify(saved));
        }
        catch { }
    }
    ...
    function renderJoin() {
        ...
        $("#language-toggle").addEventListener("click", () => { setLanguage(lang === "zh" ? "en" : "zh"); if (!params.get("name"))
            roomName = lang === "zh" ? "未命名空间" : "Untitled Space"; renderJoin(); });
        document.querySelectorAll(".saved-room-btn").forEach((btn) => btn.addEventListener("click", () => {
        ...
        $("#join-form").addEventListener("submit", async (e) => {
            ...
            const named = ($("#space-name")?.value || "").trim();
            if (named)
                roomName = named;
            else if (!roomName)
                roomName = lang === "zh" ? "未命名空间" : "Untitled Space";
            ...
            try {
                if (!roomId) {
                    ...
                    if (!created.ok)
                        throw new Error(lang === "zh" ? "无法创建空间" : "Could not create room");
                ...
                connect(() => { joined = true; renderCanvas(); }, () => {
                    joined = false;
                    renderJoin();
                    toast(lang === "zh" ? "连接失败，请重试" : "Connection failed. Please retry.");
                });
            }
            catch (error) {
                toast(error.message || (lang === "zh" ? "无法加入此空间" : "Could not join this space"));
            }
        });
    }
    function saveObjects() {
        try {
            localStorage.setItem("spatial:room:" + roomId, JSON.stringify({ objects, edges }));
        }
        catch (_) { }
    }
    function loadTrash() { try {
        return JSON.parse(localStorage.getItem("spatial:trash:" + roomId) || "[]");
    }
    catch (_) {
        return [];
    } }
    function saveTrash() { try {
        localStorage.setItem("spatial:trash:" + roomId, JSON.stringify(trashItems));
    }
    catch (_) { } }
    ...
    function syncStateToPeers(before) {
    ...
    function restoreSnapshot(next, toastKey) {
    ...
    function sanitizeRichHtml(value) {
        ...
        const clean = (parent) => {
            for (const node of Array.from(parent.childNodes)) {
                ...
                if (["SCRIPT", "STYLE", "IFRAME", "OBJECT", "EMBED", "SVG", "MATH", "FORM", "INPUT", "BUTTON"].includes(node.tagName)) {
                    node.remove();
                    continue;
                }
                clean(node);
    ...
    function richHtml(object) { return sanitizeRichHtml(object.content ?? t("richTextDefault")); }
    function isRootObject(o) { return !o.folderId; }
    function visibleObjects() {
        ...
        if (openedFolderId)
            return objects.filter((o) => o.folderId === openedFolderId);
        return objects.filter(isRootObject);
    }
    function objectById(id) { return objects.find(o => o.id === id); }
    function clearCardSelection(except) { document.querySelectorAll(".canvas-card.selected,.canvas-card.multi-selected").forEach((node) => { if (node !== except)
        node.classList.remove("selected", "multi-selected"); }); }
    function isCanvasTarget(target) { const node = target; return target === $("#canvas-wrap") || !!node?.classList?.contains("canvas-grid") || !!node?.classList?.contains("canvas-world") || !!node?.closest?.(".edges-layer"); }
    ...
    async function saveCurrentSpace() {
        try {
            ...
            toast(error.message || (lang === "zh" ? "保存失败" : "Save failed"));
        }
    }
    function bindCanvas() {
        ...
        for (const type of ["pointerdown", "click", "dblclick", "contextmenu", "keydown", "beforeinput", "paste", "drop"]) {
            document.addEventListener(type, (e) => {
                if (e instanceof KeyboardEvent && (e.key === "F5" || ((e.ctrlKey || e.metaKey) && ["r", "c", "a"].includes(e.key.toLowerCase()))))
                    return;
                if (joined && !collaborationReady) {
                    e.preventDefault();
        ...
        const refSave = document.querySelector('.reference-save');
        if (refSave)
            refSave.onclick = (e) => { e.stopPropagation(); void saveCurrentSpace(); };
        document.querySelector('.reference-actions').onclick = (e) => showWorkspaceSwitcher(e);
        ...
        if (mouseModeRoot) {
            mouseModeRoot.querySelectorAll("[data-mouse-mode]").forEach((btn) => {
                btn.onclick = (e) => {
                    e.stopPropagation();
                    const mode = (e.currentTarget.dataset.mouseMode || "click");
        ...
        wrap.addEventListener("contextmenu", (e) => { e.preventDefault(); const hit = document.elementFromPoint(e.clientX, e.clientY); const card = (hit && hit.closest(".canvas-card")) || e.target?.closest?.(".canvas-card"); if (card)
        ...
        wrap.addEventListener("auxclick", (e) => { if (e.button === 1)
            e.preventDefault(); });
        wrap.addEventListener("mousedown", (e) => { if (e.button === 1)
            e.preventDefault(); });
        wrap.addEventListener("pointermove", (e) => {
            if (pinDrag) {
                ...
                pinDrag.el.style.left = `${pinDrag.pin.x * 100}%`;
                pinDrag.el.style.top = `${pinDrag.pin.y * 100}%`;
                renderEdges();
            ...
            if (drag?.selection) {
                ...
                if (gridSnapEnabled) {
                    movingX = snapCoord(movingX);
                    movingY = snapCoord(movingY);
                }
                ...
                if (gridSnapEnabled)
                    showSnapGuides(movingX, movingY, true);
                else
                    clearSnapGuides();
        ...
        wrap.addEventListener("pointerup", (e) => {
            ...
            if (drag?.card || drag?.group)
                updateDropTarget(e, new Set(drag.group ? drag.items.map(item => item.object.id) : [drag.card.id]));
            ...
            if (tap) {
                ...
                if (drag?.group || drag?.card || drag?.resize)
                    finishDrag();
                else
                    drag = null;
                if (isTap) {
        ...
        wrap.addEventListener("pointercancel", (e) => {
            ...
            if (drag?.selection)
                finishSelection();
            else if (drag?.group || drag?.card || drag?.resize)
                finishDrag(true);
            else
                drag = null;
        });
        ...
        syncMouseModeUi();
        $("#invite-btn").onclick = () => { void copyInviteLink(); };
        const saveBtn = $("#save-btn");
        if (saveBtn)
            saveBtn.onclick = () => { void saveCurrentSpace(); };
        document.addEventListener("click", (e) => {
            ...
            if (!clickTarget || !clickTarget.isConnected || menu.contains(clickTarget) || clickTarget.closest?.(".context-menu"))
                return;
            hideContextMenu();
        }, { signal });
        document.addEventListener("keydown", (e) => {
            const editing = e.target?.closest?.("textarea,input,[contenteditable=true],.todo-item-text");
            ...
            const drawingPanel = document.querySelector('#panel-viewer.open.is-artboard');
            const textEditing = e.target?.closest?.('textarea,[contenteditable=true],input:not([type=range]):not([type=color]):not([type=button]):not([type=checkbox])');
            if (mod && !e.altKey && !e.isComposing && !textEditing && (key === 'z' || key === 'y') && drawingPanel && !document.querySelector('.spatial-modal-root.open')) {
                ...
                drawingPanel.querySelector(`[data-draw="${action}"]`)?.click();
                return;
            }
            if (e.key === "Escape") {
                if (pinDrag) {
                ...
                if (drag?.card || drag?.group || drag?.resize) {
                    e.preventDefault();
                    pendingCardTap = null;
                    finishDrag(true);
                    return;
                }
                if (document.querySelector(".spatial-modal-root.open")) {
                    e.preventDefault();
                    closeSpatialModal();
                    return;
                }
                if (document.querySelector(".folder-peek-root.open")) {
                    e.preventDefault();
                    hideFolderPeek();
                    return;
                }
            ...
            if (mod && !editing && key === "z" && !e.shiftKey) {
                e.preventDefault();
                undoLast();
                return;
            }
            if (mod && !editing && ((key === "z" && e.shiftKey) || key === "y")) {
                e.preventDefault();
                redoLast();
                return;
            }
            if (mod && !editing && key === "g") {
                e.preventDefault();
        ...
        if (!window.__spatialPasteBound) {
            window.__spatialPasteBound = true;
            document.addEventListener("paste", (e) => { void handleClipboardPaste(e); });
        }
    }
    ...
    function showUtilityMenu(e, mode) {
        ...
        menu.querySelectorAll('[data-utility]').forEach((button) => button.onclick = (event) => {
            ...
            if (['fit', 'invite', 'language'].includes(action))
                return contextAction(action);
            const folder = objectById(action);
            if (folder)
                openFolder(folder);
        });
        ...
        if (input) {
            input.oninput = () => {
                ...
                const results = menu.querySelector('.search-results');
                ...
                results.querySelectorAll('button').forEach((b) => b.onclick = () => { hideContextMenu(); const o = objectById(b.dataset.result); if (o?.kind === 'folder')
                    openFolder(o);
                else if (o)
                    openPanel(o); });
            };
    ...
    function ensureCurrentSavedIfKnown() {
        if (!roomId)
            return;
    ...
    async function createAndOpenWorkspace() {
        ...
        try {
            ...
            toast(error.message || (lang === "zh" ? "创建失败" : "Create failed"));
            location.href = `/?lang=${lang}`;
        }
    }
    function showWorkspaceSwitcher(e) {
        e.stopPropagation();
        ...
        menu.style.left = `${Math.max(12, Math.min(r.left, innerWidth - menu.offsetWidth - 12))}px`;
        menu.style.top = `${Math.max(12, r.top - menu.offsetHeight - 12)}px`;
        menu.querySelectorAll("[data-ws-room]").forEach((button) => button.onclick = (event) => {
            event.stopPropagation();
        ...
        menu.querySelectorAll("[data-ws]").forEach((button) => button.onclick = (event) => {
            event.stopPropagation();
    ...
    function collectClipboardImageFiles(data) {
        if (!data)
            return [];
        const out = [];
        const seen = new Set();
    ...
    async function readClipboardImagesAsync() {
    ...
    async function handleClipboardPaste(e) {
        ...
        if (!files.length && !editingText)
            files = await readClipboardImagesAsync();
        if (!files.length) {
    ...
    function showTrashMenu() {
        ...
        menu.innerHTML = `<div class="context-title">${icon("trash")} ${t("trash")}</div>${rows}${trashItems.length ? `<div class="context-separator"></div><button type="button" data-empty-trash>${t("emptyTrash")}</button>` : ""}`;
        menu.classList.add("open");
        document.body.classList.add("menu-open");
        menu.querySelectorAll("[data-restore]").forEach((b) => b.onclick = (event) => {
            event.stopPropagation();
        ...
        menu.querySelector("[data-empty-trash]")?.addEventListener("click", (event) => {
            event.stopPropagation();
            snapshotState();
            trashItems = [];
            saveTrash();
    ...
    function objectsInGroup(groupId) {
        if (!groupId)
            return [];
    ...
    function expandIdsWithGroups(ids) {
        ...
        for (const id of ids) {
            ...
            if (obj?.groupId)
                objectsInGroup(obj.groupId).forEach((member) => next.add(member.id));
            else
                next.add(id);
        }
        return next;
    ...
    function selectionObjects() {
        ...
        return ids.map((id) => objectById(id)).filter((o) => !!o);
    }
    ...
    function clearSnapGuides() {
        const el = $("#snap-guides");
        if (el)
            el.innerHTML = "";
    }
    function showSnapGuides(x, y, edge = false) {
        const layer = $("#snap-guides");
        const wrap = $("#canvas-wrap");
        if (!layer || !wrap)
            return;
    ...
    function alignSelection(mode) {
        ...
        if (mode === "left")
            targets.forEach((o) => { o.x = left; });
        else if (mode === "center-x")
            targets.forEach((o) => { o.x = cx - o.width / 2; });
        else if (mode === "right")
            targets.forEach((o) => { o.x = right - o.width; });
        else if (mode === "top")
            targets.forEach((o) => { o.y = top; });
        else if (mode === "center-y")
            targets.forEach((o) => { o.y = cy - o.height / 2; });
        else if (mode === "bottom")
            targets.forEach((o) => { o.y = bottom - o.height; });
    ...
    function tidySelection() {
        ...
        if (targets.length < 2) {
            ...
            if (images.length >= 2 && images.length >= Math.ceil(targets.length * 0.5))
                targets = images;
        }
        if (targets.length < 2) {
            toast(t("tidyNeedMore"));
            return;
        }
        snapshotState();
    ...
    function updateAlignBar() {
        ...
        const btn = (mode, iconName, labelKey) => `<button type="button" data-align="${mode}" title="${t(labelKey)}" aria-label="${t(labelKey)}">${icon(iconName)}</button>`;
        ...
        bar.hidden = false;
        bar.querySelectorAll("[data-align]").forEach((node) => {
            node.onclick = (ev) => {
                ev.stopPropagation();
                alignSelection(node.dataset.align);
            };
        });
        const tidyBtn = bar.querySelector("[data-tidy]");
        if (tidyBtn)
            tidyBtn.onclick = (ev) => { ev.stopPropagation(); tidySelection(); };
    }
    function startSelection(e) { selectedEdgeId = null; renderEdges(); selectionDrag = { sx: e.clientX, sy: e.clientY }; drag = { selection: true }; const box = $("#selection-box"); if (box) {
    ...
    function updateSelection(e) { if (!selectionDrag)
        return; const box = $("#selection-box"); if (!box)
        return; const left = Math.min(selectionDrag.sx, e.clientX); const top = Math.min(selectionDrag.sy, e.clientY); box.style.left = `${left}px`; box.style.top = `${top}px`; box.style.width = `${Math.abs(e.clientX - selectionDrag.sx)}px`; box.style.height = `${Math.abs(e.clientY - selectionDrag.sy)}px`; }
    function finishSelection() { const start = selectionDrag; const box = $("#selection-box"); selectionDrag = null; drag = null; if (box)
        box.classList.remove("open"); if (!start)
        return; const end = { x: Number.parseFloat(box?.style.left || start.sx), y: Number.parseFloat(box?.style.top || start.sy) }; const rect = $("#canvas-wrap")?.getBoundingClientRect(); if (!rect)
        return; const x1 = Math.min(start.sx, end.x); const y1 = Math.min(start.sy, end.y); const x2 = Math.max(start.sx, end.x + Number.parseFloat(box?.style.width || "0")); const y2 = Math.max(start.sy, end.y + Number.parseFloat(box?.style.height || "0")); const ids = visibleObjects().filter(o => { const left = rect.left + view.x + o.x * view.scale; const top = rect.top + view.y + o.y * view.scale; const right = left + o.width * view.scale; const bottom = top + o.height * view.scale; return left < x2 && right > x1 && top < y2 && bottom > y1; }).map(o => o.id); selectedIds = new Set(ids); selected = ids.length === 1 ? ids[0] : null; renderObjects(); }
    function hideFolderExit() {
        document.getElementById("folder-exit-drop")?.classList.remove("open", "is-target");
    }
    function updateDropTarget(e, excluded) {
        if (!drag)
            return;
        ...
        if (openedFolderId && moved && (drag.card || drag.group)) {
            if (!exit) {
                ...
                exit.id = "folder-exit-drop";
                exit.setAttribute("role", "status");
        ...
        if (candidate)
            document.querySelector(`[data-id="${candidate.id}"]`)?.classList.add("folder-drop-target");
    }
    function moveDraggedToParent(active) {
        ...
        while (siblings.some(o => x < o.x + o.width + 24 && x + width + 24 > o.x && y < o.y + o.height + 24 && y + height + 24 > o.y))
            y += height + 40;
        for (const item of movers) {
            item.object.folderId = parentId;
            item.object.x = x + item.ox - minX;
            item.object.y = y + item.oy - minY;
        }
        openedFolderId = parentId || null;
        selectedIds = movingIds;
        selected = movers[0].object.id;
    ...
    function fitCanvas() {
        ...
        if (!wrap || !scope.length) {
            view = { x: 0, y: 0, scale: .82 };
            return applyView();
        }
    ...
    function applyView() { const world = $("#canvas-world"); if (world)
        world.style.transform = `translate(${view.x}px,${view.y}px) scale(${view.scale})`; const zoomButton = document.querySelector('.reference-date'); if (zoomButton)
        zoomButton.textContent = Math.round(view.scale * 100) + "%"; const label = $("#zoom-label"); if (label)
        label.textContent = Math.round(view.scale * 100) + "%"; syncGridVisual(); renderRemotes(); }
    function finishDrag(cancelled = false) {
        const active = drag;
        hideFolderExit();
        if (!active || (!active.card && !active.resize && !active.group)) {
            drag = null;
            return;
        }
        if (cancelled) {
            active.exitFolder = false;
            active.dropFolderId = null;
            if (active.group)
                active.items.forEach(item => { item.object.x = item.ox; item.object.y = item.oy; });
            if (active.card) {
                active.card.x = active.ox;
                active.card.y = active.oy;
            }
            if (active.resize) {
                active.resize.x = active.ox;
                active.resize.y = active.oy;
                active.resize.width = active.ow;
                active.resize.height = active.oh;
            }
        }
        ...
        if (active.group) {
            ...
            if (active.dropFolderId) {
                ...
                active.items.forEach((item) => {
                    if (openedFolderId)
                        item.object.folderId = openedFolderId;
                });
            }
        }
        else if (active.card) {
    ...
    function updateCursor(e) {
        ...
        me.cursor = p;
        const emit = () => { lastAwarenessAt = Date.now(); awarenessTimer = null; const msg = { type: "awareness", roomId, state: { sessionId: me.id, nickname: me.name, color: me.color, cursor: me.cursor, selection: { objectId: selected || undefined }, updatedAt: Date.now() } }; const n = fanoutP2p(msg); if (!n)
            send(msg); };
        const wait = 40 - (Date.now() - lastAwarenessAt);
        if (wait <= 0)
            emit();
        else if (!awarenessTimer)
            awarenessTimer = setTimeout(emit, wait);
    }
    function orderedLayerObjects(folderId) {
    ...
    function layerActionsHtml(object) {
    ...
    function changeObjectLayer(object, action) {
        const ordered = orderedLayerObjects(object.folderId);
        const next = reorderedLayers(ordered, layerSelection(object), action);
        if (!next.some((o, index) => o.id !== ordered[index].id))
            return;
        snapshotState();
        const changed = [];
        next.forEach((o, index) => { if (o.zIndex !== index + 1) {
            o.zIndex = index + 1;
            changed.push(o);
        } });
        saveObjects();
    ...
    function syncObjectLayers() {
        ...
        world.querySelectorAll(":scope > .canvas-card").forEach(el => {
        ...
        const connectors = world.querySelector(":scope > .edges-layer");
        if (connectors)
            connectors.style.zIndex = String(ordered.length + 3);
    }
    ...
    function adoptIntoFolderLocal(movers, folderId) {
     ...
     */
    function localizeFolderChildrenIfNeeded(folderId) {
    ...
    function hideFolderPeek() {
        ...
        const root = document.querySelector(".folder-peek-root");
        if (!root)
            return;
        ...
        const finish = () => {
            if (root.isConnected && !root.classList.contains("open"))
                root.remove();
            peekCloseTimer = null;
        };
        ...
        const onEnd = (e) => {
            if (e.target !== root || (e.propertyName !== "opacity" && e.propertyName !== "transform"))
                return;
    ...
    function folderCardThumb(child) {
    ...
    function folderPeekThumb(child) {
    ...
    let peekEvents = null;
    function showFolderPeek(folder, anchorEl) {
        hideContextMenu();
        const existing = document.querySelector('.folder-peek-root.open');
        if (peekFolderId === folder.id && existing)
            return;
        hideFolderPeek();
        if (peekCloseTimer != null) {
        ...
        const anchor = anchorEl?.isConnected ? anchorEl : document.querySelector(`[data-id="${folder.id}"]`);
      ...
      <div class="folder-peek-actions"><span>${lang === 'zh' ? '双击卡片打开 · 最多预览 3 项' : 'Double-click a card · Up to 3 previews'}</span><button type="button" data-peek-enter>${t('openFolder')} ↗</button></div>
        ...
        const panel = root.querySelector('.folder-peek-panel');
        ...
        if (rect && above < 12 && below + height > innerHeight - 90) {
            if (rect.left - width - 18 >= 12)
                left = rect.left - width - 18;
            else if (rect.right + width + 18 <= innerWidth - 12)
                left = rect.right + 18;
        ...
        const enter = () => { const current = objectById(folder.id); if (current)
            openFolder(current); };
        ...
        root.querySelectorAll('[data-peek-id]').forEach(btn => {
            const open = () => {
        ...
        document.addEventListener('pointerdown', e => {
            const target = e.target;
            if (root.contains(target) || target.closest(`[data-id="${folder.id}"]`))
                return;
            hideFolderPeek();
        }, { capture: true, signal });
    ...
    function bindLinkEmbed(object) {
        const viewer = $("#panel-viewer");
        if (!viewer)
            return;
        const frame = viewer.querySelector(".link-embed-frame");
        const fallback = viewer.querySelector(".link-embed-fallback");
        const loading = viewer.querySelector(".link-embed-loading");
        const urlLabel = viewer.querySelector("[data-link-url-label]");
        const href = normalizeUrl(object.url);
        if (urlLabel)
            urlLabel.textContent = href || (object.url || "");
        const openExt = () => { if (href)
            window.open(href, "_blank", "noopener,noreferrer"); };
        viewer.querySelectorAll("[data-link-external]").forEach((btn) => {
            btn.onclick = (e) => { e.preventDefault(); e.stopPropagation(); openExt(); };
        });
        viewer.querySelectorAll("[data-link-edit]").forEach((btn) => {
            btn.onclick = (e) => { e.preventDefault(); e.stopPropagation(); openLinkEditSheet(object); };
        });
        viewer.querySelector("[data-link-reload]")?.addEventListener("click", (e) => {
            e.preventDefault();
            e.stopPropagation();
            if (!frame || !href)
                return;
                ...
                if (doc && (!doc.body || doc.body.childElementCount === 0) && !(doc.body?.innerText || "").trim()) {
                    ...
                    setTimeout(() => {
                        try {
                            ...
                            if (again && (!again.body || (!(again.body.innerText || "").trim() && again.body.childElementCount === 0)))
                                showFallback();
                            else
                                showOk();
                        }
    ...
    function openPanel(object) {
        ...
        if (object.kind === "todo") {
            ...
            if ((object.folderId || null) !== openedFolderId) {
                openedFolderId = object.folderId || null;
                renderObjects();
                fitCanvas();
            }
            document.querySelector(`[data-id="${object.id}"] [data-todo-add]`)?.focus();
            return;
        }
        const viewer = $("#panel-viewer");
        if (!viewer)
            return;
        viewer._lightboxAbort?.abort();
        ...
        openPanelId = object.id;
        viewer.innerHTML = panelMarkup(object);
        viewer.classList.add("open");
        viewer.setAttribute("aria-hidden", "false");
        document.body.classList.add("panel-open");
        viewer.querySelector("[data-panel-close]")?.addEventListener("click", closePanel);
        viewer.querySelectorAll("[data-panel-action]").forEach((button) => button.addEventListener("click", () => panelAction(button.dataset.panelAction, object)));
        if (object.kind === "artboard")
            bindDrawingPanel(object);
        if (object.kind === "image")
            bindImageLightbox(object);
        if (object.kind === "link")
            bindLinkEmbed(object);
        if (object.kind === "richText") {
            bindRichCard(object, viewer);
            syncRichEditor(object, viewer);
        }
    }
    function bindDrawingPanel(object) {
        ...
        const undoButton = toolbar.querySelector('[data-draw="undo"]');
        const redoButton = toolbar.querySelector('[data-draw="redo"]');
        ...
        const paint = () => {
            ...
            for (const stroke of [...(object.strokes || []), ...(active ? [active] : [])]) {
                if (!stroke.points?.length)
                    continue;
                const pts = stroke.points, a = pts[0], b = pts[pts.length - 1];
                ctx.save();
                ctx.globalCompositeOperation = stroke.tool === 'eraser' ? 'destination-out' : 'source-over';
                ctx.globalAlpha = stroke.tool === 'marker' ? .3 : 1;
                ...
                if (stroke.tool === 'rect')
                    ...
                    if (pts.length === 1) {
                        ctx.arc(a.x, a.y, stroke.size / 2, 0, Math.PI * 2);
                        ctx.fill();
                    }
                }
                ctx.stroke();
        ...
        canvas.onpointerdown = (e) => { if (e.button !== 0)
            return; e.preventDefault(); canvas.setPointerCapture(e.pointerId); active = { tool: mode, points: [point(e)], color: toolbar.querySelector('.drawing-color').value, size: Number(toolbar.querySelector('.drawing-size').value) * (mode === 'marker' ? 3 : mode === 'eraser' ? 4 : 1) }; paint(); };
        canvas.onpointermove = (e) => { if (!active)
            return; active.points.push(point(e)); paint(); };
        canvas.onpointerup = () => { if (!active)
            return; object.strokes.push(active); active = null; redo = []; commit(); };
        ...
        toolbar.querySelectorAll('[data-draw]').forEach((button) => button.onclick = () => {
            ...
            if (action === 'undo') {
                if (object.strokes.length)
                    redo.push(object.strokes.pop());
                commit();
            }
            else if (action === 'redo') {
                if (redo.length)
                    object.strokes.push(redo.pop());
                commit();
            }
            else if (action === 'trash') {
                if (confirm(zh ? '清空这张画板？' : 'Clear this artboard?')) {
                    object.strokes = [];
                    redo = [];
                    commit();
                }
                ...
                mode = action;
                toolbar.querySelectorAll('[data-draw]').forEach(b => b.setAttribute('aria-pressed', String(b === button)));
            }
        });
        ...
        canvas.repaintDrawing = paint;
        paint();
    }
    function bindImageLightbox(_object) {
        const viewer = $("#panel-viewer");
        const img = viewer?.querySelector(".lightbox-img");
        const viewport = viewer?.querySelector(".lightbox-viewport");
        if (!viewer || !img || !viewport)
            return;
        const ac = new AbortController();
        viewer._lightboxAbort = ac;
        const { signal } = ac;
        ...
        if (toolbar && !toolbar.querySelector("[data-lightbox-zoom]")) {
            ...
            toolbar.querySelectorAll("[data-lightbox-zoom]").forEach((button) => button.addEventListener("click", (e) => {
                e.stopPropagation();
        ...
        viewer.addEventListener("click", (e) => {
            ...
            if (!target)
                return;
            if (target.closest(".lightbox-img, .panel-toolbar, .panel-back, .panel-meta, a, button, [data-lightbox-zoom-label]"))
                return;
            if (target === viewer || target.classList.contains("panel-viewer-shell") || target.classList.contains("panel-content") || target.classList.contains("lightbox-viewport") || target.classList.contains("panel-art") || target.classList.contains("lightbox-stage"))
                closePanel();
        }, { signal });
        document.addEventListener("keydown", (e) => {
            if (!viewer.classList.contains("open") || !viewer.classList.contains("is-image"))
                return;
    ...
    function closePanel() {
        ...
        if (viewer.classList.contains("is-rich")) {
            ...
            const object = objectById(openPanelId);
            if (object) {
                flushTextUpdate(object);
                syncObjectDom(object);
            }
        ...
        setTimeout(() => { if (!viewer.classList.contains("open"))
            viewer.innerHTML = ""; }, 340);
        const closedId = openPanelId;
        openPanelId = null;
        document.body.classList.remove("panel-open");
        if (closedId) {
            const closed = objects.find((o) => o.id === closedId);
            if (closed?.kind === "artboard")
                scheduleArtboardThumbnail(closed);
        }
    }
    function panelAction(action, object) {
        if (action === "edit") {
            return contextAction("edit", object);
        }
        if (action === "duplicate") {
            closePanel();
            return contextAction("duplicate", object);
        }
        if (action === "delete") {
            closePanel();
            return contextAction("delete", object);
        }
        if (action === "download") {
            if (object.kind === "artboard") {
                const canvas = document.querySelector(".drawing-canvas");
    ...
    function applyRemoteEdges(incoming, deletedEdges = [], opts = {}) {
        if (!incoming && !(deletedEdges && deletedEdges.length) && !opts.replaceAll)
            return;
    ...
    function applyRemoteObjects(nextObjects, deletedObjects = [], opts = {}) {
        ...
        for (let incoming of list) {
            if (!incoming?.id || deletedObjects.includes(incoming.id))
                continue;
            if (incoming.kind === "richText" && richPending.get(incoming.id) === incoming.content) {
                richPending.delete(incoming.id);
                if (richDrafts.get(incoming.id) === incoming.content)
                    richDrafts.delete(incoming.id);
            }
            if (incoming.kind === "richText" && richTitlePending.get(incoming.id) === incoming.title) {
                richTitlePending.delete(incoming.id);
                if (richTitleDrafts.get(incoming.id) === incoming.title)
                    richTitleDrafts.delete(incoming.id);
            }
            ...
            if (dragging.has(incoming.id)) {
                if (incoming.kind === "todo")
                    Object.assign(current, { title: incoming.title, items: incoming.items, todoVersion: incoming.todoVersion });
                if (incoming.kind === "richText")
                    Object.assign(current, { title: incoming.title, content: incoming.content, richTextVersion: incoming.richTextVersion });
                merged.push(current);
                continue;
            }
            ...
            if (localText != null)
                current.text = localText;
            if (localHtml !== undefined)
                current.content = localHtml;
            if (richTitleDrafts.has(incoming.id))
                current.title = richTitleDrafts.get(incoming.id);
            merged.push(current);
        }
        ...
        saveObjects();
        if (openedFolderId && !objectById(openedFolderId))
            openedFolderId = null;
        if (openPanelId && !objectById(openPanelId))
            closePanel();
        if (peekFolderId && !objectById(peekFolderId))
            hideFolderPeek();
        const visible = visibleObjects();
        ...
        world?.querySelectorAll(":scope > .canvas-card").forEach(el => {
            if (!visibleIds.has(el.dataset.id)) {
                ...
                if (deleted.has(id)) {
                    richDrafts.delete(id);
                    richPending.delete(id);
                    richTitleDrafts.delete(id);
                    richTitlePending.delete(id);
                }
                el.remove();
            }
        });
        for (const o of visible) {
            if (!world?.querySelector(`[data-id="${o.id}"]`)) {
                world?.insertAdjacentHTML("beforeend", cardHtml(o));
                bindObject(o);
            }
            else
                syncObjectDom(o, dragging);
        }
        // A drawing viewer can remain open while its card is inside a folder.
        if (openPanelId) {
            const o = objectById(openPanelId);
            if (o)
                syncOpenPanel(o);
        }
        updateFolderBreadcrumb();
    ...
    function paintStrokes(ctx, strokes, scaleX = 1, scaleY = 1) {
    ...
    function scheduleArtboardThumbnail(o) {
        if (o.kind !== "artboard")
            return;
        ...
        artboardThumbTimers.set(o.id, window.setTimeout(() => {
            ...
            const preview = document.querySelector(`[data-id="${o.id}"] .artboard-preview`);
            if (!preview)
                return;
            if (url)
                preview.innerHTML = `<img class="artboard-thumb" src="${url}" alt="" draggable="false">`;
            else
                preview.innerHTML = `<div class="artboard-empty"><span class="artboard-empty-icon">${icon("pen")}</span><span>${escapeHtml(t("emptyArtboard"))}</span><small>${escapeHtml(t("artboardEmptyHint"))}</small></div>`;
        }, 160));
    }
    function kindLabel(kind) { return ({ note: t("sticky"), richText: t("richText"), image: t("image"), link: t("link"), folder: t("folder"), artboard: t("artboard"), todo: t("todo"), pin: lang === "zh" ? "图钉" : "Pin" })[kind] || kind; }
    function portPoint(o, port = "right") {
        if (o.kind === "pin")
            return { x: o.x + o.width / 2, y: o.y + o.height / 2 };
        if (port.startsWith("pin:")) {
            const pin = o.pins?.find(p => p.id === port.slice(4));
            if (pin)
                return { x: o.x + pin.x * o.width, y: o.y + pin.y * o.height };
        }
        if (port === "left")
    ...
    function bezierPath(a, b, fromPort, toPort) {
    ...
    function ensureEdgesLayer() {
        ...
        if (!world)
            return null;
        let svg = world.querySelector(":scope > .edges-layer");
        if (!svg) {
    ...
    function renderEdgePreview() { renderEdges(); }
    function createsParentCycle(fromId, toId, ignoredEdgeId) {
        if (fromId === toId)
            return true;
        const outgoing = new Map();
        for (const edge of edges) {
            if (edge.id === ignoredEdgeId || edge.relation !== "parent")
                continue;
            const targets = outgoing.get(edge.fromId) || [];
            targets.push(edge.toId);
            outgoing.set(edge.fromId, targets);
        }
        const remaining = [toId], seen = new Set();
        while (remaining.length) {
            ...
            if (seen.has(id))
                continue;
            seen.add(id);
            remaining.push(...(outgoing.get(id) || []));
        }
        return false;
    }
    function setEdgeRelation(edgeId, action) {
        const edge = edges.find(item => item.id === edgeId);
        if (!edge)
            return;
        const next = { ...edge, relation: action === "related" ? "related" : "parent" };
        if (action === "to-parent") {
            next.fromId = edge.toId;
            next.toId = edge.fromId;
            next.fromPort = edge.toPort;
            next.toPort = edge.fromPort;
        }
        ...
        if (next.relation === (edge.relation || "related") && next.fromId === edge.fromId && next.toId === edge.toId)
            return;
        snapshotState();
    ...
    function showEdgeContextMenu(x, y, edgeId) {
        ...
        const objectName = (id) => { const object = objectById(id); return object ? (object.title || object.caption || kindLabel(object.kind)) : id; };
        const names = `${objectName(edge.fromId)} → ${objectName(edge.toId)}`;
        const option = (action, label, active) => `<button data-edge-relation="${action}" role="menuitemradio" aria-checked="${active}" class="edge-relation-option${active ? " is-active" : ""}"><span class="context-leading">${icon(action === "related" ? "link" : "back")}</span><span>${label}</span><span class="edge-relation-check" aria-hidden="true">${active ? "✓" : ""}</span></button>`;
        ...
        menu.querySelectorAll("[data-edge-relation]").forEach(button => button.addEventListener("click", () => {
            hideContextMenu();
            setEdgeRelation(edgeId, button.dataset.edgeRelation);
        }));
    ...
    function createEdge(fromId, toId, fromPort, toPort) {
        ...
        if (edges.some(e => (e.fromId === fromId && e.toId === toId && e.fromPort === fp && e.toPort === tp) || (e.fromId === toId && e.toId === fromId && e.fromPort === tp && e.toPort === fp)))
            return null;
        const edge = {
            id: uid(),
            fromId,
    ...
    function deleteSelectedObjects() {
        ...
        for (const id of ids) {
            ...
            if (object.kind === "folder")
                objects.forEach((child) => { if (child.folderId === object.id)
    ...
    function finishConnDrag(e) {
        ...
        const portEl = hit?.closest?.(".conn-port");
        const card = hit?.closest?.(".canvas-card");
        let toId = null;
        let toPort;
        if (portEl && card) {
            toId = card.dataset.id || null;
            toPort = (portEl.dataset.port || "left");
        }
        else if (card) {
            toId = card.dataset.id || null;
        }
        renderEdges();
        if (toId && toId !== fromId)
            createEdge(fromId, toId, fromPort, toPort);
    }
    function todoProgressMeta(o) {
        const items = o.items || [];
        const total = items.length;
        const done = items.filter((it) => it.done).length;
        return { done, total, pct: total ? Math.round((done / total) * 100) : 0 };
    }
    function todoProgressHtml(o) {
    ...
    function patchTodoProgress(root, o) {
    ...
    function sendTodoOperation(o, operation) {
    ...
    function patchTodoCard(o, el) {
        const title = el.querySelector("[data-todo-title]");
        if (title && (document.activeElement !== title || title.value === title.dataset.originalValue)) {
            title.value = o.title || t("todoTitle");
            title.dataset.originalValue = title.value;
        }
        ...
        list.querySelectorAll("[data-todo-id]").forEach(row => {
            if (!ids.has(row.dataset.todoId))
                row.remove();
        });
        if (!ids.size) {
            list.innerHTML = todoItemsHtml(o);
        }
        else {
            list.querySelector(".todo-empty")?.remove();
            for (const item of o.items || []) {
                let row = Array.from(list.querySelectorAll("[data-todo-id]")).find(r => r.dataset.todoId === item.id);
                if (!row) {
                ...
                const input = row.querySelector("[data-todo-text]");
                if (document.activeElement !== input || input.value === input.dataset.originalValue) {
    ...
    function bindTodoCard(o, el) {
        ...
        el.addEventListener("focusout", e => {
            ...
            if (field.value !== field.dataset.originalValue) {
                if (field.matches("[data-todo-title]"))
                    sendTodoOperation(o, { action: "title", text: field.value });
                if (field.matches("[data-todo-text]"))
                    sendTodoOperation(o, { action: "text", itemId: field.closest("[data-todo-id]")?.dataset.todoId, text: field.value });
            }
            if (editingObjectId === o.id)
                setEditingObject(null);
        });
        el.addEventListener("click", e => {
            const button = e.target.closest("[data-todo-toggle],[data-todo-remove]");
            if (!button)
                return;
            e.preventDefault();
            e.stopPropagation();
        ...
        el.addEventListener("keydown", e => {
            const field = e.target;
            if (e.key !== "Enter" || e.isComposing || !field.matches("input"))
                return;
            e.preventDefault();
            e.stopPropagation();
            if (field.matches("[data-todo-add]")) {
                const text = field.value.trim();
                if (text && sendTodoOperation(o, { action: "add", itemId: uid(), text }))
                    field.value = "";
            }
            else {
                field.blur();
                el.querySelector("[data-todo-add]")?.focus();
            }
        });
    }
    function portsHtml() {
    ...
    function syncCardPins(o, el) {
        const signature = JSON.stringify(o.pins || []);
        if (el.dataset.pins === signature)
            return;
        ...
        el.querySelectorAll(":scope > .card-pin").forEach(port => bindConnectionPort(o, el, port));
    }
    function addCardPin(o) {
        if (o.kind === "pin")
            return;
    ...
    function showPinMenu(o, port, e) {
        ...
        menu.querySelector("[data-remove-pin]")?.addEventListener("click", () => {
            ...
            syncObjectDom(o);
            renderEdges();
            sendUpdate([o], [], [], removed);
        });
    }
    function bindConnectionPort(o, el, port) {
        port.addEventListener("pointerdown", (e) => {
            ...
            const name = (port.dataset.port || "right");
            drag = null;
            pendingCardTap = null;
            suppressCardClick = true;
            const pin = name.startsWith("pin:") && o.pins?.find(p => p.id === name.slice(4));
            if (pin && e.altKey)
                pinDrag = { object: o, pin, el: port, x: pin.x, y: pin.y, before: cloneCanvasState() };
            else {
                document.body.classList.add("connecting");
                const point = worldPoint(e);
                connDrag = { fromId: o.id, fromPort: name, x: point.x, y: point.y };
            }
            selected = o.id;
        ...
        if (port.classList.contains("card-pin"))
            port.addEventListener("contextmenu", e => { e.preventDefault(); e.stopPropagation(); showPinMenu(o, port.dataset.port, e); });
    }
    function finishPinDrag(cancelled = false) {
        const active = pinDrag;
        if (!active)
            return;
        pinDrag = null;
        if (cancelled) {
            active.pin.x = active.x;
            active.pin.y = active.y;
        }
        else if (active.pin.x !== active.x || active.pin.y !== active.y) {
            undoStack.push(active.before);
            if (undoStack.length > 30)
                undoStack.shift();
            redoStack = [];
        }
        active.el.style.left = `${active.pin.x * 100}%`;
        active.el.style.top = `${active.pin.y * 100}%`;
        saveObjects();
        syncObjectDom(active.object);
        renderEdges();
        sendUpdate([active.object]);
    }
    function resizeHandlesHtml() {
        const labels = lang === "zh" ? ["左上", "右上", "左下", "右下"] : ["top left", "top right", "bottom left", "bottom right"];
        return ["nw", "ne", "sw", "se"].map((corner, index) => `<button type="button" class="resize-handle" data-resize-corner="${corner}" tabindex="-1" aria-label="${lang === "zh" ? `从${labels[index]}调整大小` : `Resize from ${labels[index]}`}" title="${lang === "zh" ? "拖动调整大小" : "Drag to resize"}"></button>`).join("");
    }
    function todoItemsHtml(o) {
        const items = o.items || [];
        if (!items.length) {
    ...
    function cardHtml(o) {
        ...
        if (o.kind === "pin")
            return `<article class="canvas-card pin-card${active}" data-id="${o.id}" aria-label="${kindLabel("pin")}" style="left:${o.x}px;top:${o.y}px;width:${o.width}px;height:${o.height}px">${portsHtml()}<span class="pin-head">${icon("pin")}</span></article>`;
    ...
    function bindRichCard(o, el) {
        const rich = el.querySelector(".rich-editor");
        const title = el.querySelector(".doc-title");
        let savedRange = null;
        let typingFont = null;
        let titleTimer = null;
        const sizeSelect = el.querySelector(".font-size-select");
        const smaller = el.querySelector('[data-font-step="-1"]');
        const larger = el.querySelector('[data-font-step="1"]');
        let currentFontSize = parseFloat(getComputedStyle(rich).fontSize) || 18;
        const showFontSize = (size) => {
            currentFontSize = size;
            sizeSelect.value = RICH_FONT_SIZES.includes(size) ? String(size) : "";
            smaller.disabled = size <= RICH_FONT_SIZES[0];
            larger.disabled = size >= RICH_FONT_SIZES[RICH_FONT_SIZES.length - 1];
        };
            ...
            if (selection?.rangeCount && rich.contains(selection.anchorNode) && rich.contains(selection.focusNode)) {
                ...
                if (node.nodeType === Node.ELEMENT_NODE && node.childNodes.length) {
                    ...
                    while (node.childNodes.length)
                        node = atEnd ? node.lastChild : node.firstChild;
                }
        ...
        const restoreSelection = () => {
            rich.focus();
            const selection = window.getSelection();
            if (savedRange && rich.contains(savedRange.commonAncestorContainer)) {
        ...
        const changed = () => {
            ...
            if (content.length > 200000) {
                rich.innerHTML = richHtml(o);
                toast(lang === "zh" ? "文档过长，请拆分为多个富文本面板" : "Please split this document into smaller panels");
                return;
            }
            o.content = content;
            setEditingObject(o.id);
            scheduleTextUpdate(o);
            rememberSelection();
            syncRichStatus(o, el);
            syncObjectDom(o);
        };
        ...
        rich.addEventListener("click", e => { if (e.target.closest("a"))
            e.preventDefault(); });
        ...
        title.addEventListener("input", () => {
            o.title = title.value;
            richTitleDrafts.set(o.id, title.value);
            syncRichStatus(o, el);
            syncObjectDom(o);
            if (titleTimer)
        ...
        title.addEventListener("blur", () => {
            ...
            if (title.value !== title.dataset.originalValue) {
                o.title = title.value;
                richTitleDrafts.set(o.id, title.value);
                flushRichDraft(o);
                saveObjects();
                ...
                title.value = o.title ?? t("newDocument");
            syncRichStatus(o, el);
            syncObjectDom(o);
        });
        title.addEventListener("keydown", e => { if (e.key === "Enter" && !e.isComposing) {
            e.preventDefault();
            title.blur();
        } });
        const applyFontSize = (size) => {
            const index = RICH_FONT_SIZES.indexOf(size);
            if (index < 0)
                return;
            restoreSelection();
        ...
        el.querySelectorAll(".fmt-btn").forEach(btn => {
            ...
            btn.addEventListener("click", e => {
                ...
                if (cmd === "createLink") {
                    ...
                    if (!href) {
                        toast(lang === "zh" ? "请输入有效的 http、https 或邮件链接" : "Use a valid http, https or email link");
                        return;
                    }
                    value = href;
                }
                ...
                if (cmd === "removeFormat")
                    typingFont = null;
                if (cmd === "formatBlock")
                    value = document.queryCommandValue("formatBlock").toLowerCase() === "h2" ? "p" : "h2";
                document.execCommand(cmd, false, value);
                changed();
            });
        });
    }
    function syncRichStatus(o, root) {
        const status = root.querySelector(".rich-save-status");
        if (status)
            status.textContent = !collaborationReady ? (lang === "zh" ? "正在重连…" : "Reconnecting…") : richDrafts.has(o.id) || richTitleDrafts.has(o.id) ? (lang === "zh" ? "正在保存…" : "Saving…") : (lang === "zh" ? "已保存" : "Saved");
        const count = root.querySelector(".rich-word-count");
        const text = root.querySelector(".rich-editor")?.textContent || "";
        if (count)
            count.textContent = lang === "zh" ? `${text.replace(/\s/g, "").length} 字` : `${text.trim() ? text.trim().split(/\s+/).length : 0} words`;
    }
    function syncRichEditor(o, root) {
        const title = root.querySelector(".doc-title");
        if (title && !richTitleDrafts.has(o.id) && (document.activeElement !== title || title.value === title.dataset.originalValue)) {
            title.value = o.title ?? t("newDocument");
            title.dataset.originalValue = title.value;
        }
        const rich = root.querySelector(".rich-editor");
        const content = richHtml(o);
        if (rich && !richDrafts.has(o.id) && sanitizeRichHtml(rich.innerHTML) !== content)
    ...
    function bindObject(o) {
        ...
        el.querySelectorAll("[data-upload-retry]").forEach((btn) => {
            btn.onclick = (e) => { e.preventDefault(); e.stopPropagation(); void retryImageUpload(o.id); };
        });
        el.querySelectorAll(":scope > .conn-port").forEach(port => bindConnectionPort(o, el, port));
        syncCardPins(o, el);
        ...
        if (o.kind === 'folder') {
            ...
            el.addEventListener('keydown', (event) => { if (event.key === 'Enter' || event.key === ' ') {
                event.preventDefault();
                showFolderPeek(o, el);
            } });
            el.addEventListener('dblclick', (event) => { event.preventDefault(); event.stopPropagation(); if (openedFolderId !== o.id)
                openFolder(o); });
        }
        if (o.kind === "todo")
            bindTodoCard(o, el);
        el.addEventListener("pointerdown", (e) => {
            ...
            if (o.kind === "folder") {
                ...
                selected = o.id;
                el.classList.add("selected");
                pendingCardTap = { id: o.id, el: el, sx: e.clientX, sy: e.clientY, pointerId: e.pointerId, slop: allowDrag ? 8 : 32, moved: false };
                if (allowDrag) {
                    pendingDragSnapshot = cloneCanvasState();
                    drag = { card: o, el: el, sx: e.clientX, sy: e.clientY, ox: o.x, oy: o.y };
                    $("#canvas-wrap")?.setPointerCapture(e.pointerId);
            ...
            selected = o.id;
            pendingCardTap = { id: o.id, el: el, sx: e.clientX, sy: e.clientY, pointerId: e.pointerId, slop: allowDrag ? 8 : 20, moved: false };
            el.classList.add("selected");
            if (!allowDrag) {
            ...
            const items = members.map(object => ({ object, el: document.querySelector(`[data-id="${object.id}"]`), ox: object.x, oy: object.y })).filter((item) => !!item.el);
            pendingDragSnapshot = cloneCanvasState();
            drag = items.length > 1 ? { group: true, items, sx: e.clientX, sy: e.clientY } : { card: o, el: el, sx: e.clientX, sy: e.clientY, ox: o.x, oy: o.y };
            $("#canvas-wrap")?.setPointerCapture(e.pointerId);
        ...
        if (editor) {
            ...
            editor.addEventListener("blur", () => { if (editingObjectId === o.id)
                setEditingObject(null); flushTextUpdate(o); });
            editor.addEventListener("input", (e) => { o.text = e.target.value; setEditingObject(o.id); scheduleTextUpdate(o); });
        }
        el.querySelectorAll("[data-resize-corner]").forEach(handle => {
            handle.addEventListener("pointerdown", (e) => {
                ...
                pendingDragSnapshot = cloneCanvasState();
                drag = {
                    resize: o, resizeCorner: handle.dataset.resizeCorner, el,
                ...
                $("#canvas-wrap")?.setPointerCapture(e.pointerId);
                el.classList.add("selected", "dragging", "resizing");
                updateAlignBar();
            });
            handle.addEventListener("dblclick", e => { e.preventDefault(); e.stopPropagation(); });
        });
    }
    function pickImageFiles(multiple = false) {
        return new Promise((resolve) => {
            ...
            input.addEventListener("change", () => {
                const files = Array.from(input.files || []).filter((f) => f.type.startsWith("image/") || /\.(png|jpe?g|gif|webp|bmp|svg)$/i.test(f.name));
                finish(files);
            });
    ...
    function fitImageSize(naturalW, naturalH, maxW = 360, maxH = 280) {
                ...
                try {
                    ...
                    canvas.toBlob((webp) => {
                        if (webp && webp.size < file.size * 0.85 && webp.size < tw * th) {
                            // Also compare JPEG
                            canvas.toBlob((jpg) => {
                                if (jpg && jpg.size < webp.size)
                                    finish(jpg, "image/jpeg", ".jpg");
                                else
                                    finish(webp, "image/webp", ".webp");
    ...
    function uploadImageAssetWithProgress(file, onProgress) {
    ...
    async function addImageFromFile(file, at, offsetIndex = 0) {
        try {
            ...
            toast(error.message || (lang === "zh" ? "图片添加失败" : "Could not add image"));
            return null;
        }
    }
    async function finalizeImageUpload(o, file, previewUrl) {
        const paint = () => syncObjectDom(o);
        try {
            o.uploadProgress = Math.max(o.uploadProgress || 0, 6);
            paint();
            const prepared = await compressImageForUpload(file);
            o._pendingFile = prepared;
            ...
            sendUpdate([o]);
            try {
                if (previewUrl.startsWith("blob:") && o.src !== previewUrl)
    ...
    async function retryImageUpload(objectId) {
        const o = objectById(objectId);
        if (!o || o.kind !== "image")
            return;
        const file = o._pendingFile;
        if (!file) {
            toast(t("uploadFailed"));
            return;
        }
    ...
    async function addImagesFromFiles(files, at, opts) {
    ...
    async function promptAddImage(at) {
    ...
    function addObject(kind, at) {
        ...
        const o = kind === "artboard" ? { ...base, kind, title: t("artboard"), strokes: [] }
    ...
    function contextAction(action, object) {
        ...
        if (object && action?.startsWith("layer-"))
            return changeObjectLayer(object, action);
        if (action === "image") {
            void promptAddImage(contextPoint);
            return;
        }
        if (action === "folder" || action === "artboard" || action === "todo" || action === "richText" || action === "pin")
            return addObject(action, contextPoint);
        if (action === "note" || action === "link")
            return;
        if (action === "fit")
            return fitCanvas();
        if (action === "invite") {
        ...
        if (!object)
            return;
        if (action === "add-card-pin")
            return addCardPin(object);
        if (action === "open")
            return object.kind === "folder" ? openFolder(object) : openPanel(object);
        if (action === "delete") {
            snapshotState();
            if (object.kind === "folder")
                objects.forEach(child => { if (child.folderId === object.id)
        ...
        if (action === "duplicate") {
            snapshotState();
            const clone = { ...structuredClone(wireObject(object)), id: uid(), x: object.x + 28, y: object.y + 28 };
            objects.push(clone);
            selected = clone.id;
            saveObjects();
            renderObjects();
            sendUpdate([clone]);
            return toast(t("duplicated"));
        }
        if (action === "edit") {
            selected = object.id;
            if (object.kind === "image")
                return openImageEditSheet(object);
            if (object.kind === "link")
                return openLinkEditSheet(object);
            if (object.kind === "folder")
                return openFolderEditSheet(object);
            if (object.kind === "richText") {
                openPanel(object);
                document.querySelector("#panel-viewer .rich-editor")?.focus();
                return;
            ...
            if (object.kind === "todo") {
                document.querySelector(`[data-id="${object.id}"] [data-todo-title]`)?.focus();
                return;
            }
    ...
    function closeSpatialModal() {
        ...
        window.setTimeout(() => { if (!root.classList.contains("open"))
            root.remove(); }, 200);
    }
    function ensureSpatialModalRoot() {
        let root = document.querySelector(".spatial-modal-root");
        if (!root) {
    ...
    function openSpatialSheet(panelClass, title, bodyHtml) {
        const root = ensureSpatialModalRoot();
        const panel = root.querySelector(".spatial-modal");
    ...
    function openImageEditSheet(object) {
        const caption0 = object.caption || t("newVisual");
        const panel = openSpatialSheet("image-edit-sheet", t("editImageTitle"), `<div class="image-edit-preview"><img class="image-edit-preview-img" src="${escapeHtml(object.src || "")}" alt="${escapeHtml(caption0)}" draggable="false"><div class="image-edit-caption-live">${escapeHtml(caption0)}</div></div>
      ...
      <button type="button" class="spatial-btn primary" data-apply-image>${escapeHtml(t("apply"))}</button></div>`);
        ...
        const img = panel.querySelector(".image-edit-preview-img");
        const live = panel.querySelector(".image-edit-caption-live");
        const captionInput = panel.querySelector("[data-caption-input]");
        const fileInput = panel.querySelector(".spatial-file-input");
        const dropzone = panel.querySelector(".spatial-dropzone");
        ...
        const adoptFile = (file) => {
            ...
            if (!captionInput.value.trim() || captionInput.value === (object.caption || t("newVisual"))) {
                captionInput.value = (file.name || t("newVisual")).replace(/\.[^.]+$/, "") || t("newVisual");
                syncLive();
            }
        };
        ...
        const onDrag = (e) => { e.preventDefault(); e.stopPropagation(); dropzone.classList.add("is-dragover"); };
        ...
        panel.querySelector("[data-apply-image]")?.addEventListener("click", async () => {
            const btn = panel.querySelector("[data-apply-image]");
            btn.disabled = true;
            try {
                snapshotState();
                ...
                if (pendingFile) {
                    ...
                    object._pendingFile = pendingFile;
                ...
                toast(error.message || (lang === "zh" ? "更新失败" : "Update failed"));
                btn.disabled = false;
            }
        });
    }
    function openLinkEditSheet(object) {
        const panel = openSpatialSheet("link-edit-sheet", t("editLinkTitle"), `<label class="spatial-field">${escapeHtml(t("linkTitleLabel"))}<input class="spatial-input" type="text" value="${escapeHtml(object.title || t("savedLink"))}" autocomplete="off" data-edit-link-title></label>
      ...
      <button type="button" class="spatial-btn primary" data-apply-link>${escapeHtml(t("apply"))}</button></div>`);
        const titleInput = panel.querySelector("[data-edit-link-title]");
        const urlInput = panel.querySelector("[data-edit-link-url]");
        const descInput = panel.querySelector("[data-edit-link-desc]");
    ...
    function openFolderEditSheet(object) {
        const panel = openSpatialSheet("folder-edit-sheet", t("editFolderTitle"), `<label class="spatial-field">${escapeHtml(t("folderTitleLabel"))}<input class="spatial-input" type="text" value="${escapeHtml(object.title || t("folder"))}" autocomplete="off" data-folder-title></label>
      <div class="spatial-actions"><button type="button" class="spatial-btn ghost" data-modal-dismiss>${escapeHtml(t("cancel"))}</button>
      <button type="button" class="spatial-btn primary" data-apply-folder>${escapeHtml(t("apply"))}</button></div>`);
        const titleInput = panel.querySelector("[data-folder-title]");
        ...
        panel.querySelector("[data-apply-folder]")?.addEventListener("click", () => {
            ...
            sendUpdate([object]);
            closeSpatialModal();
            toast(t("folderUpdated"));
        });
    }
    function renderPresence() { const stack = $("#presence-stack"); if (!stack)
        return; const people = [...remote, { name: me.name, color: me.color }]; stack.innerHTML = people.slice(0, 5).map(p => `<span class="presence-avatar" style="background:${p.color}">${escapeHtml((p.name || "?").slice(0, 1).toUpperCase())}</span>`).join(""); const label = $("#online-label"); if (label)
        label.textContent = people.length === 1 ? t("onePerson") : `${people.length} ${t("people")}`; }
    ...
    function iceServers() {
        ...
        try {
            const custom = window.__SPATIAL_ICE_SERVERS__;
    ...
    function updateP2pStatus() {
        ...
        else if (p2pMode === "relay")
            el.textContent = t("p2pRelay");
        else
            el.textContent = "";
        if (sep)
            sep.hidden = !el.textContent;
    }
    function teardownPeer(sessionId) {
    ...
    function sendSignal(toSessionId, payload) {
        send({ type: "signal", roomId, fromSessionId: me.id, toSessionId, payload });
    }
    function attachDataChannel(sessionId, link, dc) {
        link.dc = dc;
    ...
    function handleCollabMessage(m, fromP2p = false) {
        if (m.type === "joined") {
            if (m.sessionId)
                me.id = m.sessionId;
            ...
            if (Array.isArray(m.state?.edges))
                edges = m.state.edges;
            else if (!serverHasDoc && edges.length)
                sendUpdate([], [], edges, []);
            remote = (m.participants || []).map((p) => ({ id: p.sessionId, name: p.nickname, color: p.color, cursor: p.cursor, selection: p.selection?.objectId, updatedAt: Date.now() }));
            const peerIds = Array.isArray(m.peerSessionIds)
                ? m.peerSessionIds
                : (m.participants || []).map((p) => p.sessionId).filter(Boolean);
        ...
        if (m.type === "y-update") {
            ...
            if (Array.isArray(m.objects))
                applyRemoteObjects(m.objects, deleted, { delta });
            else if (m.update?.objects) {
                const objs = Array.isArray(m.update.objects) ? m.update.objects : Object.values(m.update.objects);
                applyRemoteObjects(objs, deleted, { delta: delta || !!(deleted && deleted.length) });
            }
            else if (deleted?.length)
                applyRemoteObjects([], deleted, { delta: true });
            applyRemoteEdges(Array.isArray(m.edges) ? m.edges : (Array.isArray(m.update?.edges) ? m.update.edges : undefined), Array.isArray(m.deletedEdges) ? m.deletedEdges : (Array.isArray(m.update?.deletedEdges) ? m.update.deletedEdges : []), { replaceAll: Array.isArray(m.edges) && !fromP2p && !m.live });
        }
    }
    function locallyDraggingIds() {
        const ids = new Set();
        if (pinDrag)
            ids.add(pinDrag.object.id);
        if (drag?.card)
            ids.add(drag.card.id);
        if (drag?.resize)
            ids.add(drag.resize.id);
        if (drag?.group && drag.items)
            drag.items.forEach(item => ids.add(item.object.id));
        return ids;
    }
    function syncOpenPanel(o) {
        if (openPanelId !== o.id)
            return;
        const viewer = $("#panel-viewer");
        ...
        if (o.kind === "artboard") {
            viewer.querySelector(".drawing-canvas")?.repaintDrawing?.();
            const meta = viewer.querySelector(".panel-meta span");
            if (meta)
                meta.textContent = `${t("artboard")} · ${o.strokes?.length || 0}`;
            return;
        }
        if (o.kind === "note") {
            const article = viewer.querySelector(".panel-note");
            if (article)
                article.innerHTML = escapeHtml(o.text || "").replace(/\n/g, "<br>");
            return;
        }
        if (o.kind === "richText") {
        ...
        if (o.kind === "link") {
            ...
            const h1 = viewer.querySelector(".panel-meta h1");
            if (h1)
                h1.textContent = o.title || t("savedLink");
            return;
        }
        if (o.kind === "folder") {
            const strong = viewer.querySelector(".panel-folder strong");
            if (strong)
                strong.textContent = o.title || t("folder");
            const small = viewer.querySelector(".panel-folder small");
            if (small)
                small.textContent = `${objects.filter(child => child.folderId === o.id).length} ${t("items")}`;
        }
    }
    function syncObjectDom(o, skipIds) {
        if (skipIds?.has(o.id)) {
            if (o.kind === "todo") {
                const card = document.querySelector(`[data-id="${o.id}"]`);
                if (card)
                    patchTodoCard(o, card);
            }
            return;
        }
        const el = document.querySelector(`[data-id="${o.id}"]`);
        ...
        if (o.kind === "note") {
            ...
            const ta = el.querySelector("textarea.note-editor");
            if (ta && !editingThis && ta.value !== (o.text || ""))
                ta.value = o.text || "";
        }
        else if (o.kind === "richText") {
            const docTitle = el.querySelector(".image-caption-text");
            if (docTitle)
                docTitle.textContent = o.title || t("newDocument");
            const rich = el.querySelector(".rich-preview");
            const nextHtml = richHtml(o);
            if (rich && rich.innerHTML !== nextHtml)
                rich.innerHTML = nextHtml;
        }
            ...
            if (img && o.src && img.getAttribute("src") !== o.src)
                img.src = o.src;
            if (img)
                img.alt = o.caption || t("newVisual");
            const capText = el.querySelector(".image-caption-text");
            if (capText)
                capText.textContent = o.caption || t("newVisual");
            else {
                const cap = el.querySelector(".image-caption");
                if (cap)
                    cap.innerHTML = `<span class="image-caption-text">${escapeHtml(o.caption || t("newVisual"))}</span>`;
            }
            ...
            if (bar)
                bar.style.width = `${Math.max(0, Math.min(100, o.uploadProgress || 0))}%`;
            const overlay = el.querySelector(".upload-overlay");
            if (overlay)
                overlay.hidden = !o.uploading && !o.uploadError;
            ...
            if (name)
                name.textContent = o.title || t("folder");
            const count = el.querySelector(".folder-count");
            if (count)
                count.textContent = `${objects.filter(child => child.folderId === o.id).length} ${t("items") || ""}`;
            ...
            if (preview) {
                ...
                el.classList.toggle("has-contents", children.length > 0);
                el.classList.toggle("is-empty", children.length === 0);
            }
        }
        else if (o.kind === "artboard") {
            const name = el.querySelector(".artboard-name");
            if (name)
                name.textContent = o.title || t("artboard");
            const count = el.querySelector(".artboard-count");
            if (count)
                count.textContent = String(o.strokes?.length || 0);
    ...
    function scheduleLiveMoveBroadcast() {
        const emit = () => {
        ...
        if (wait <= 0)
            emit();
        else if (!liveMoveTimer)
            liveMoveTimer = setTimeout(emit, wait);
    }
    ...
    function scheduleReconnect() {
        if (leavingPage || !joined || reconnectTimer)
            return;
        updateConnectionBanner();
        const delay = Math.min(15000, 1000 * 2 ** Math.min(reconnectAttempt++, 4));
        reconnectTimer = setTimeout(() => { reconnectTimer = null; connect(); }, delay);
    }
    function connect(onJoined, onJoinFailed) {
        let settled = false;
        let timer = null;
        const fail = () => {
            if (settled)
        ...
        try {
            ...
            connection.onmessage = (e) => {
                if (socket !== connection || (settled && onJoinFailed && !joined))
                    return;
                try {
    ...
    function send(msg) { if (socket?.readyState === 1)
    ...
    function sendLiveUpdate(changedObjects) {
        const list = changedObjects.filter(Boolean);
        if (!list.length)
            return;
        const msg = { type: "y-update", roomId, objects: list.map(wireObject), deletedObjects: [], live: true };
        const n = fanoutP2p(msg);
        if (!n)
            send(msg);
    }
    function sendUpdate(changedObjects = [], deletedObjects = [], changedEdges = [], deletedEdges = []) {
        ...
        if (!list.length && !deleted.length && !edgeList.length && !edgeDeleted.length)
            return;
        ...
        if (list.length || deleted.length)
            fanoutP2p({ ...msg, edges: [], deletedEdges: [] });
        send(msg);
    }
    ...
    window.addEventListener('keydown', e => { if (e.key === 'Alt')
        document.body.classList.add('mouse-alt-drag'); });
    window.addEventListener('keyup', e => { if (e.key === 'Alt')
        document.body.classList.remove('mouse-alt-drag'); });
    window.addEventListener('blur', () => document.body.classList.remove('mouse-alt-drag'));
    window.addEventListener("beforeunload", () => {
        document.activeElement?.matches("#panel-viewer.is-rich .doc-title") && document.activeElement.blur();
        for (const object of objects)
            if (object.kind === "richText")
                flushRichDraft(object);
        leavingPage = true;
    ...
    window.addEventListener("online", () => { if (joined && !collaborationReady)
        connect(); });
    setInterval(renderRemotes, 1000);
    if (!restoreRoomSession())
        renderJoin();
})();
